...
func _on_Timer_timeout():
	...
	if len(overlap_bodies) > 0:
		...
		if winner_color != null:
			get_tree().root.get_node("MAIN/WORLD/CHARACTER/BODY").GET_BRIDGE("L_RET3", winner_color, 0.01)
			get_tree().root.get_node("MAIN/WORLD/EYES_VIEW/L3").color = winner_color

	else:
		$Timer.stop()


func _on_L_RET3_body_entered(body):
	$Timer.start()




func _on_L_RET3_body_exited(body):
	if len(get_overlapping_bodies()) == 0:
		get_tree().root.get_node("MAIN/WORLD/EYES_VIEW/L3").color = Color(00,00,00)
