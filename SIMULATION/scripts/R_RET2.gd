...
func _on_Timer_timeout():
	...
	if len(overlap_bodies) > 0:
		...
		if winner_color != null:
			get_tree().root.get_node("MAIN/WORLD/CHARACTER/BODY").GET_BRIDGE("R_RET2", winner_color, 0.01)
			get_tree().root.get_node("MAIN/WORLD/EYES_VIEW/R2").color = winner_color

	else:
		#print("Остановка мониторинга")
		$Timer.stop()

func _on_R_RET2_body_entered(body):
	get_tree().root.get_node("MAIN/WORLD/CHARACTER/BODY").GET_BRIDGE("R_RET2", body.self_color, 0.01)
	get_tree().root.get_node("MAIN/WORLD/EYES_VIEW/R2").color = body.self_color
	$Timer.start()


func _on_R_RET2_body_exited(body):
	if len(get_overlapping_bodies()) == 0:
		get_tree().root.get_node("MAIN/WORLD/EYES_VIEW/R2").color = Color(00,00,00)
