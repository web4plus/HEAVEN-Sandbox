...
var RayCastList = ["Ray1", "Ray2", "Ray3", "Ray4", "Ray5", "Ray6", "Ray7", "Ray8", "Ray9"]
...
func _on_Timer_timeout():
	...
	if len(overlap_bodies) > 0:
		...
		for ray in RayCastList:
			...
			#get_tree().root.get_node("MAIN").ADD_TO_CONSOLE(ray_obj)
			#var ray_point = ray_obj.get_collision_point()
			#get_tree().root.get_node("MAIN").ADD_TO_CONSOLE(ray_point)
		...
		if winner_color != null:
			get_tree().root.get_node("MAIN/WORLD/CHARACTER/BODY").GET_BRIDGE("R_RET3", winner_color, 0.01)
			get_tree().root.get_node("MAIN/WORLD/EYES_VIEW/R3").color = winner_color
...
func _on_R_RET3_body_entered(body):
	$Timer.start()



func _on_R_RET3_body_exited(body):
	if len(get_overlapping_bodies()) == 0:
		get_tree().root.get_node("MAIN/WORLD/EYES_VIEW/R3").color = Color(00,00,00)
